...
// GET /api/driver/registration/status - Get registration status
router.get("/registration/status", async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
    ...
    // Determine driver type from user role
    const isDeliveryDriver = user.role === "delivery_driver" || user.role === "pending_delivery_driver";
    ...
    res.json({
      profile: {
        id: profile.id,
        driverType: isDeliveryDriver ? "delivery" : "ride",
    ...
    console.error("Driver registration status error:", error);
    res.status(500).json({ error: "Failed to get registration status" });
  }
});

// Schema for driver registration submission
...
// POST /api/driver/registration/submit - Submit driver registration
router.post("/registration/submit", async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: "Unauthorized" });
    ...
    const { driverType, personalInfo, vehicleInfo, documents } = parsed.data;
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (driverProfile) {
        ...
        driverProfile = await tx.driverProfile.update({
          ...
          data: {
            ...
            nidFrontImageUrl: documents.nidFrontImageUrl || null,
            nidBackImageUrl: documents.nidBackImageUrl || null,
            ...
            driverLicenseImageUrl: documents.licenseImageUrl || null,
      ...
      // Update user role to pending driver
    ...
    res.json({
      ...
      message: isBD 
        ...
        : "Driver application submitted. Please wait for approval.",
      profile: {
        id: result.id,
    ...
    console.error("Driver registration submit error:", error);
    res.status(500).json({ error: "Failed to submit driver registration" });
  }
});

// POST /api/driver/registration/save-step - Auto-save registration step
router.post("/registration/save-step", async (req: AuthRequest, res) => {
  try {
    const userId = req.user?.userId;
    if (!userId) {
      return res.status(401).json({ error: "Unauthorized" });
    ...
    // Store partial data in driver profile
    let updateData: any = {};

    // Map step data to profile fields
    if (data.personalInfo) {
      const pi = data.personalInfo;
      if (pi.phone) updateData.phoneNumber = pi.phone;
      if (pi.dateOfBirth) updateData.dateOfBirth = new Date(pi.dateOfBirth);
      if (pi.emergencyContactName) updateData.emergencyContactName = pi.emergencyContactName;
      if (pi.emergencyContactPhone) updateData.emergencyContactPhone = pi.emergencyContactPhone;
      if (pi.fatherName) updateData.fatherName = pi.fatherName;
      if (pi.presentAddress) updateData.presentAddress = pi.presentAddress;
      if (pi.permanentAddress) updateData.permanentAddress = pi.permanentAddress;
      if (pi.homeAddress) updateData.homeAddress = pi.homeAddress;
    }

    if (data.documents) {
      const docs = data.documents;
      if (docs.nidNumber) updateData.nidNumber = docs.nidNumber;
      if (docs.driverLicenseNumber) updateData.driverLicenseNumber = docs.driverLicenseNumber;
      if (docs.governmentIdType) updateData.governmentIdType = docs.governmentIdType;
      if (docs.governmentIdLast4) updateData.governmentIdLast4 = docs.governmentIdLast4;
      if (docs.ssnLast4) updateData.ssnLast4 = docs.ssnLast4;
    }
    ...
    console.error("Driver registration save-step error:", error);
