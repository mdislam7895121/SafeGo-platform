...
import { customerSupportService } from "../services/GenericSupportService";
...
router.get("/support-center/tickets", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const tickets = await customerSupportService.listTickets(context.profileId);
...
router.get("/support-center/tickets/:id", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const ticket = await customerSupportService.getTicketById(ticketId, context.profileId);
...
router.post("/support-center/live-chat/start", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const session = await customerLiveChatService.startSession({
      profileId: context.profileId,
      customerName: context.displayName,
      initialMessage
    });
...
router.get("/support-center/live-chat/:sessionId", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const session = await customerLiveChatService.getSession(sessionId, context.profileId);
...
router.post("/support-center/live-chat/:sessionId/messages", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const message = await customerLiveChatService.sendMessage({
      sessionId,
      profileId: context.profileId,
...
router.post("/support-center/live-chat/:sessionId/end", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const session = await customerLiveChatService.endSession(sessionId, context.profileId);
    return res.json({ session });
  } catch (error: any) {
...
router.post("/support-center/callbacks", async (req: AuthRequest, res: Response) => {
  try {
    ...
    const callback = await customerCallbackService.requestCallback({
      profileId: context.profileId,
      phoneNumber,
      preferredTime,
      timezone,
