...
-   **Cross-State Fare Engine**: Uber-style dedicated pricing for interstate trips that completely overrides normal fare calculation when pickup and dropoff states differ. Uses specific rates: base fare ($3.00), per-mile rate ($2.00), per-minute rate ($0.60), cross-state surcharge ($2.00), with configurable surge multiplier. Includes comprehensive UI breakdown showing base fare, distance cost, time cost, tolls, cross-state surcharge, surge multiplier, and total fare. Logs cross-state pricing triggers for debugging (NJ to NY, NY to NJ, PA to NJ, NY to NY scenarios).
