...
-   **Restaurant Analytics & Management Systems**: Includes comprehensive Performance Insights (R2) with KPI tracking (Orders, Earnings, AOV, Acceptance Rate, Cancellation Rate, Completed Orders), time-series charts (Orders Over Time, Earnings Over Time), top categories/items analysis, and recent orders table with mobile-first responsive design and time range controls (today/7d/30d). Also includes Review & Rating Management, Media Gallery & Branding Management, and Operational Settings System (business hours, delivery/pickup settings, delivery zones, surge pricing).
