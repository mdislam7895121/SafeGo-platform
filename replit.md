...
- **Unified Eats Integration on /customer**: The Eats tab in the unified booking page now shows the full restaurant browsing experience instead of a placeholder. CustomerEatsHome component (`client/src/components/customer/CustomerEatsHome.tsx`) embeds the restaurant list with search, filters (cuisine, open now, favorites), and sorting. Cart badge with item count appears in header when in Eats mode. Navigation flows: browse restaurants → view menu/add to cart → checkout → order tracking. Uses existing EatsCartContext for cart state management.
...
- **Multiple Delivery Address Profiles**: CustomerAddress model with labeled addresses (Home/Work/Other), lat/lng coordinates, isDefault flag, and delivery instructions. Full CRUD backend at `/api/customer/food/addresses` with auto-selection of default address at checkout.
- **Advanced Promo Code System**: Supports percentage, flat amount, and free delivery discount types. Coupon validation endpoint at `/api/coupons/validate` with restaurant-specific, usage limits, and expiry checks. Frontend EatsCartContext tracks promoFreeDelivery flag and applies discounts in getTotals().
- **Real-Time Menu Stock Sync**: MenuItem model includes stockTrackingEnabled, stockQuantity, and lowStockThreshold fields. Stock validation at order creation blocks out-of-stock items. Stock auto-decrements on order placement and restores on cancellation.
- **Order Cancellation with Automatic Refunds**: Dedicated `/api/food-orders/:id/cancel` endpoint handles customer cancellations with status-based refund logic (100% for placed/accepted, 80% for preparing). Wallet refunds processed automatically for online payments. Stock restored on cancellation. Frontend cancel button with confirmation dialog in order tracking page.
