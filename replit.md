...
-   **Unified Payout System**: Production-ready payout infrastructure for all user roles (Restaurant, Driver, Customer) with unified `/api/payout/*` routes protected by RBAC. Features include: automatic weekly payout scheduling, bank verification service with Bangladesh (NID/Passport + business license) and USA (SSN/EIN + routing validation) KYC rules, country-specific minimum payout amounts (BD: 500 BDT, US: $10, others: $20), payout method management (bank_transfer, mobile_money, cash), comprehensive withdrawal request validation, full transaction history with audit trails, and seamless integration with existing Wallet + Earnings infrastructure maintaining backwards compatibility.
