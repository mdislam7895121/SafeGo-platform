...
import { UtensilsCrossed, Eye, Clock, Search as SearchIcon, CheckCircle, XCircle, Clock4, Filter } from "lucide-react";
...
export default function AdminOnboardingRestaurants() {
  ...
  if (cuisineFilter !== "all") queryParams.append("cuisineType", cuisineFilter);
  queryParams.append("page", currentPage.toString());
  queryParams.append("limit", "20");

  const queryString = queryParams.toString();
  const fullUrl = `/api/partner-onboarding/restaurants${queryString ? `?${queryString}` : ''}`;

  const { data, isLoading } = useQuery<ApplicationsResponse>({
    queryKey: ['/api/partner-onboarding/restaurants', statusFilter, cityFilter, searchQuery, cuisineFilter, currentPage],
    queryFn: async () => {
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Select value={cuisineFilter} onValueChange={(v) => { setCuisineFilter(v); setCurrentPage(1); }}>
            ...
            <Table>
              ...
              <TableBody>
                {data.applications.map((app) => {
                  ...
                  return (
                    <TableRow key={app.id} data-testid={`row-restaurant-${app.id}`}>
                      ...
                      <TableCell>
                        <Badge variant="outline">{app.cuisineType}</Badge>
                      </TableCell>
                      ...
                      <TableCell className="text-right">
                        <Button
                          variant="outline"
                          size="sm"
                          onClick={() => navigate(`/admin/onboarding/restaurants/${app.id}`)}
