...
const TYPE_CONFIG: Record<string, { icon: typeof Car; label: string }> = {
  drivers: { icon: Car, label: "Driver/Courier" },
  restaurants: { icon: UtensilsCrossed, label: "Restaurant" },
  shops: { icon: Store, label: "Shop" },
  tickets: { icon: Ticket, label: "Ticket Partner" }
};

export default function AdminOnboardingCenterDetail() {
  const [, navigate] = useLocation();
  const [, params] = useRoute("/admin/onboarding-center/:type/:id");
  ...
  if (!application) {
    return (
      <div className="space-y-6">
        <Button variant="ghost" onClick={() => navigate("/admin/onboarding-center")} data-testid="button-back">
          <ChevronLeft className="mr-2 h-4 w-4" />
          Back to Onboarding Center
        </Button>
        <Card>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-muted-foreground">{typeConfig.label} Application</p>
          </div>
        </div>
