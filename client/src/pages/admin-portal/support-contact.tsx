...
export default function SupportContact() {
  ...
  const { data: ticketsData, isLoading } = useQuery<{ tickets: any[] }>({
    queryKey: ["/api/admin/support-center/tickets"],
  });

  const createTicketMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      return apiRequest("/api/admin/support-center/tickets", {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/support-center/tickets"] });
