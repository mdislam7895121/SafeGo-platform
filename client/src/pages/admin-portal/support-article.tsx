...
export default function SupportArticle() {
  ...
  const { data: articleData, isLoading } = useQuery<{ article: any }>({
    queryKey: ["/api/admin/support-center/articles", id],
    enabled: !!id,
  });
    ...
    <div className="min-h-screen bg-background">
      <div className="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8 py-8 space-y-6">
        ...
        <div className="flex items-center gap-2 text-sm text-muted-foreground">
          <Link href="/admin/support/help" className="hover:text-foreground">
            Help Center
          </Link>
          ...
          <Card>
            ...
            <CardContent className="space-y-2">
              {article.relatedArticles.map((related: any) => (
                <Link key={related.id} href={`/admin/support/articles/${related.id}`}>
                  <div 
