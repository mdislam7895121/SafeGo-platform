...
import { ArrowLeft, Wallet, DollarSign, FileText, TrendingUp, Send } from "lucide-react";
...
export default function DriverWallet() {
  ...
  const { data, isLoading } = useQuery<DriverWalletResponse>({
  ...
  const payoutMutation = useMutation({
    mutationFn: async (amount: number) => {
      const res = await apiRequest("POST", "/api/driver/payout/request", { amount });
      return res.json();
    },
    onSuccess: () => {
      toast({
