...
export default function RestaurantSupportTicketDetail() {
  ...
  const { data: ticket, isLoading, error } = useQuery<Ticket>({
    queryKey: ["/api/driver/support/tickets", ticketId],
    queryFn: async () => {
      const response = await fetch(`/api/driver/support/tickets/${ticketId}`);
  ...
  const replyMutation = useMutation({
    mutationFn: async () => {
      return apiRequest(`/api/driver/support/tickets/${ticketId}/messages`, {
        method: "POST",
        body: JSON.stringify({
    ...
    onSuccess: () => {
      ...
      setReplyMessage("");
      queryClient.invalidateQueries({ queryKey: ["/api/driver/support/tickets", ticketId] });
    },
    onError: (error: Error) => {
      toast({
  ...
  const statusMutation = useMutation({
    mutationFn: async () => {
      return apiRequest(`/api/driver/support/tickets/${ticketId}/status`, {
        method: "PATCH",
        body: JSON.stringify({
  ...
  const proposedResolutionMutation = useMutation({
    mutationFn: async () => {
      return apiRequest(`/api/driver/support/tickets/${ticketId}/proposed-resolution`, {
        method: "POST",
        body: JSON.stringify({
          proposedResolution,
