...
export default function RestaurantSupportTickets() {
  ...
  const { data, isLoading, error } = useQuery<{
    ...
    queryKey: ["/api/driver/support/tickets", statusFilter, priorityFilter],
    queryFn: async () => {
      const params = new URLSearchParams();
      if (statusFilter !== "all") params.append("status", statusFilter);
      if (priorityFilter !== "all") params.append("priority", priorityFilter);
      
      const response = await fetch(`/api/driver/support/tickets?${params.toString()}`);
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
            {data?.tickets.map((ticket) => (
              <Link
                key={ticket.id}
                href={`/driver/support-tickets/${ticket.id}`}
              >
                <Card
