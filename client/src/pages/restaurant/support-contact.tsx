...
export default function SupportContact() {
  ...
  const { data: ticketsData, isLoading } = useQuery<{ tickets: any[] }>({
    queryKey: ["/api/restaurant/support-center/tickets"],
  });

  const createTicketMutation = useMutation({
    mutationFn: async (data: typeof formData) => {
      return apiRequest("/api/restaurant/support-center/tickets", {
        method: "POST",
        body: JSON.stringify(data),
      });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/restaurant/support-center/tickets"] });
